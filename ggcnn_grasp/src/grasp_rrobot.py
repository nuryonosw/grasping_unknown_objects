...
class GraspIt:
    def __init__(self):
        ...
        self.trans_y = []
        self.trans_z = []
        self.grasp_temp = Grasp()
        self.grasps = []
        self.pick_goal = PickupGoal()
        self.pick_as = SimpleActionClient('/pickup', PickupAction)
        self.pick_as.wait_for_server()
        self.point_cloud = PointCloud2()
        self.obj_det = BoundingBoxes()
    ...
    def get_grasp(self):
        ...
        for i in range(len(maxes[0])):
            ...
            # Check if grasp is within object bounding box.
            if pred_x > self.obj_xmin and pred_x < self.obj_xmax and pred_y < self.obj_ymax and pred_y > self.obj_ymin:
    ...
    def get_object(self):
        # Use YOLO to get objects bounding box and position.
        self.obj_det = rospy.wait_for_message('/darknet_ros/bounding_boxes', BoundingBoxes)
        self.point_cloud = rospy.wait_for_message('/xtion/depth_registered/points', PointCloud2)
        for l in range(len(self.obj_det.bounding_boxes)):
            # Give the class name detected by YOLO.
            if self.obj_det.bounding_boxes[l].Class == 'bowl':
                self.obj_xmax = self.obj_det.bounding_boxes[l].xmax
                self.obj_xmin = self.obj_det.bounding_boxes[l].xmin
                self.obj_ymax = self.obj_det.bounding_boxes[l].ymax
                self.obj_ymin = self.obj_det.bounding_boxes[l].ymin
                centre_x = (self.obj_det.bounding_boxes[l].xmin + self.obj_det.bounding_boxes[l].xmax) / 2
                centre_y = (self.obj_det.bounding_boxes[l].ymin + self.obj_det.bounding_boxes[l].ymax) / 2
                break
        gen = pc2.read_points(self.point_cloud, skip_nans=False, field_names=("x", "y", "z"))
        gen_list = list(gen)

        centre_x = np.round(centre_x).astype(np.int)
        centre_y = np.round(centre_y).astype(np.int)
        arrayPos = centre_y*self.point_cloud.width + centre_x
        arrayPosX = arrayPos
        arrayPosY = arrayPos + 4
        arrayPosZ = arrayPos + 8
        self.obj_pos.header.stamp = rospy.Time.now()
        self.obj_pos.header.frame_id = 'xtion_rgb_optical_frame'
        self.obj_pos.pose.position.x = gen_list[arrayPosX][0]
        self.obj_pos.pose.position.y = gen_list[arrayPosY][1]
        self.obj_pos.pose.position.z = gen_list[arrayPosZ][2]

        if math.isnan(self.obj_pos.pose.position.x):
            for n in range(100):
                cx_l = centre_x + n
                arrayPos = centre_y*self.point_cloud.width + cx_l
                arrayPosX = arrayPos
                arrayPosY = arrayPos + 4
                arrayPosZ = arrayPos + 8
                self.obj_pos.pose.position.x = gen_list[arrayPosX][0]
            ...
            for r in range(100):
                cx_r = centre_x - n
                arrayPos = centre_y*self.point_cloud.width + cx_r
                arrayPosX = arrayPos
                arrayPosY = arrayPos + 4
                arrayPosZ = arrayPos + 8
                self.obj_pos.pose.position.x = gen_list[arrayPosX][0]
            ...
            for s in range(100):
                cy_d = centre_y - s
                arrayPos = cy_d*self.point_cloud.width + centre_x
                arrayPosX = arrayPos
                arrayPosY = arrayPos + 4
                arrayPosZ = arrayPos + 8
                self.obj_pos.pose.position.x = gen_list[arrayPosX][0]
            ...
            for t in range(100):
                cy_u = centre_y - t
                arrayPos = cy_u*self.point_cloud.width + centre_x
                arrayPosX = arrayPos
                arrayPosY = arrayPos + 4
                arrayPosZ = arrayPos + 8
                self.obj_pos.pose.position.x = gen_list[arrayPosX][0]
                ...
                if not math.isnan(self.x):
                    break
        self.obj_pos.pose.orientation.x = 0
        self.obj_pos.pose.orientation.y = 0
        self.obj_pos.pose.orientation.z = 0
        self.obj_pos.pose.orientation.w = 1
        self.obj_pose = PoseStamped()
        self.obj_pose = self.transform_grasp(self.obj_pos)
        self.scene_object.pose = self.obj_pose
        # Give objects dimensions to add to planning scene.
        self.scene_object.dimensions = ()
