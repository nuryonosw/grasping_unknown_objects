...
from ggcnn_predict import predict
...
points_pub = rospy.Publisher('ggcnn/img/points', Image, queue_size=1)

# Initialise some globals.
prev_mp = np.array([150, 150])

# Get the camera parameters
camera_info_msg = rospy.wait_for_message('/xtion/depth_registered/camera_info', CameraInfo)
K = camera_info_msg.K
fx = K[0]
cx = K[2]
fy = K[4]
...
def depth_callback(depth_message):
    ...
    with TimeIt('Control'):
        ...
        max_pixel = (((np.array(max_pixel) * crop_size) / out_size) + np.array([(480 - crop_size)//2 - crop_offset, (640 - crop_size) // 2]))
    ...
    with TimeIt('Publish'):
        ...
        points_pub.publish(bridge.cv2_to_imgmsg(points_out))

        # Output the best grasp pose relative to camera.
        cmd_msg = Float32MultiArray()
        cmd_msg.data = [max_pixel[0], max_pixel[1], z, ang, width, depth_center]
        cmd_pub.publish(cmd_msg)


depth_sub = rospy.Subscriber('/xtion/depth/image', Image, depth_callback, queue_size=1)
